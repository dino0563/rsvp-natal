...
<body>
    ...
    <script>
        ...
        document.getElementById('toggleOffline').onclick = async () => {
            try {
                const res = await fetch('/checkin/cache.json', {
                ...
                log('Gagal load cache: ' + e.message, 'err');
            }
        };
        ...
        async function handleScan(payload) {
            ...
            try {
                const preview = await callVerify({
                    payload,
                    preview: true
                });
        ...
        document.getElementById('btnCancel').onclick = () => {
            // tutup modal TANPA menyentuh server
            pending = null;
            // reset 'last' supaya QR yang sama bisa dipindai lagi
            last = '';
            document.getElementById('modal').style.display = 'none';
            log('Dibatalkan. Tidak ada perubahan status.', 'warn');
        };
        ...
        document.getElementById('btnConfirm').onclick = async () => {
            ...
            try {
                ...
                } else if ((res.status || '').toUpperCase() === 'DUPLICATE') {
                    log(res.msg || 'Duplikat dalam 10 detik.', 'warn');
                } else {
                ...
                pending = null;
                // setelah confirm, izinkan scan lagi QR yang sama jika perlu
                last = '';
                document.getElementById('modal').style.display = 'none';
            }
        };
